...
RUN mvn package -q -pl qrs-pgclient -am

FROM openjdk:11.0.6-jdk-slim
WORKDIR /quarkus
COPY --from=maven /quarkus/qrs-pgclient/target/lib lib
COPY --from=maven /quarkus/qrs-pgclient/target/qrs-pgclient-1.0-SNAPSHOT-runner.jar app.jar
